 ...
 */
routes.get('/', (req, res) => {
  res.render('index', { title: config.get('appConfig.appName'), 'version': pjson.version});
});
...
routes.get('/trade/:id', (req, res) => {
  if(!req.params.id || isNaN(req.params.id)) {
    ...
    tradeService.getById(parseInt(req.params.id)).then( 
      response =>  res.end(JSON.stringify(response.message))
    ).catch(err => {
      console.error(err)
...
routes.post('/trade/update/:id', (req, res) => {  
  if(!req.params.id || isNaN(req.params.id)) {
    ...
    tradeService.update(parseInt(req.params.id), req.body).then( response => {
      res.end(JSON.stringify(response))
    }).catch(err => {
      console.error(err)
      if(err.error.endsWith('- not found'))
...
routes.delete('/trade/delete/:id', (req, res) => {
  if(!req.params.id || isNaN(req.params.id)) {
    ...
    tradeService.removeById(parseInt(req.params.id)).then( response => {
      res.end(JSON.stringify(response))
    }).catch(err => {
      console.error(err)
      if(err.error.endsWith('- not found'))
